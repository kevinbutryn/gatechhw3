...
def em(algo,X_train, X_test, y_train, y_test, init_means, no_iter = 1000, component_list = [3,4,5,6,7,8,9,10,11], num_class = 7, toshow = 1):
...
def kmeans(algo,X_train, X_test, y_train, y_test, init_means, no_iter = 1000, component_list =[3,4,5,6,7,8,9,10,11], num_class = 7, toshow=  1):
	...
	plt.legend(['homogenity','completeness','silhoutette'])
	plt.xlabel('Number of clusters')
	plt.title('Performance scores for KMeans - ' + algo)


	fig5, ax5 = plt.subplots()
	ax5.plot(component_list, array_var)
	plt.title('Variance per cluster for KMeans - ' + algo)
